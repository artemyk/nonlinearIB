...
class Net(object):
    def __init__(self, encoder_arch, decoder_arch, init_beta = 0.0, trainable_sigma = True, log_sigma2 = 0., log_eta2 = 0.):
        ...
        if trainable_sigma:
            ...
            self.log_sigma2 = tf.get_variable('log_sigma2', dtype=tf.float32, trainable=True, initializer=float(log_sigma2))
            #self.allownoise = tf.get_variable('allownoise', dtype=tf.float32, trainable=False, initializer=0.0)
        else:
        ...
        noise         = tf.random_normal(shape=tf.shape(self.encoder[-1]), mean=0.0, stddev=sigma, dtype=tf.float32)
        self.T        = self.encoder[-1] + noise
        #self.T        = self.encoder[-1] + self.allownoise * noise
        self.decoder  = [self.T,]
        ...
        self.H_T_lb = entropy.GMM_entropy(self.distance_matrix, 
                                       tf.log(tf.exp(self.log_sigma2) + tf.exp(self.log_eta2)), hiddenD, 'lower')
        ...
        vl = [v for v in tf.trainable_variables() if v != self.log_sigma2]
        self.no_log_sigma2_trainstep  = self.adam_optimizer.minimize(self.VIB_loss, var_list=vl)
